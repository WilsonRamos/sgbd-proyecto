...
public:
    ...
    // ========== CLOCK EQUIVALENTES A LRU OPERATIONS 20-21 ==========
    ...
    void clockPageOperations() {
        ...
        std::cout << "\n=== OPERACIONES DE PÁGINAS CON CLOCK BUFFER ===" << std::endl;
        ...
        std::cout << "\n🕐 Solicitando página " << page_id 
                  ...
                  << " con Clock Algorithm" << std::endl;
        
        auto block = clock_buffer_manager->fetchPage(page_id);
        if (block) {
            std::cout << "\n✅ Página cargada exitosamente con Clock!" << std::endl;
            std::cout << "📄 Información del bloque:" << std::endl;
            block->displayInfo();
            
            // Mostrar estado del clock después del fetch
            std::cout << "\n🕐 Estado del Clock después del fetch:" << std::endl;
            clock_buffer_manager->displayCompactState();
            ...
            if (input == "s" || input == "S") {
                ...
                // Mostrar estado del clock después del unpin
                std::cout << "\n🕐 Estado del Clock después del unpin:" << std::endl;
                clock_buffer_manager->displayCompactState();
            }
        } else {
            std::cout << "❌ Error cargando página con Clock Algorithm" << std::endl;
        }
    }
    
    void createNewClockPage() {
        if (!clock_buffer_manager) {
            std::cout << "❌ Error: Primero inicializa Clock Buffer Manager (opción 25)" << std::endl;
            return;
        }
        
        std::cout << "\n=== CREAR NUEVA PÁGINA CON CLOCK BUFFER ===" << std::endl;
        
        int new_page_id;
        auto block = clock_buffer_manager->newPage(new_page_id);
        if (block) {
            std::cout << "\n✨ Nueva página creada con Clock Algorithm!" << std::endl;
            std::cout << "🆔 Page ID: " << new_page_id << std::endl;
            
            // Mostrar información del bloque
            std::cout << "\n📄 Información del bloque creado:" << std::endl;
            block->displayInfo();
            
            // Mostrar estado del clock después de la creación
            std::cout << "\n🕐 Estado del Clock después de crear página:" << std::endl;
            clock_buffer_manager->displayCompactState();
            ...
            // Preguntar si marcar como dirty antes de unpin
            std::cout << "\n¿Marcar página como dirty antes de liberar? (s/n): ";
            std::string input;
            std::getline(std::cin, input);
            
            bool mark_dirty = (input == "s" || input == "S");
            clock_buffer_manager->unpinPage(new_page_id, mark_dirty);
            
            std::cout << "📍 Página liberada (dirty=" << (mark_dirty ? "YES" : "NO") << ")" << std::endl;
            
            // Estado final del clock
            std::cout << "\n🕐 Estado final del Clock:" << std::endl;
            clock_buffer_manager->displayCompactState();
            
        } else {
            std::cout << "❌ Error creando nueva página con Clock Algorithm" << std::endl;
        }
    }
...
void showMenu() {
    ...
    std::cout << "Sistema con Buffer Pool (LRU) + Clock Algorithm (Equivalencias)" << std::endl;
    ...
    std::cout << "20. Operaciones páginas LRU" << std::endl;
    std::cout << "21. Crear nueva página LRU" << std::endl;
    std::cout << "22. Estado Buffer Pool" << std::endl;
    std::cout << "23. Flush páginas LRU" << std::endl;
    ...
    std::cout << "28. Operaciones páginas Clock   " << std::endl;
    std::cout << "29. Crear nueva página Clock    " << std::endl;
    std::cout << "30. Operaciones Clock avanzadas" << std::endl;
    std::cout << "31. Estado Clock Buffer        " << std::endl;
    std::cout << "32. Flush páginas Clock     " << std::endl;
    std::cout << "33. Comparar Clock vs LRU" << std::endl;
...
int main() {
    ...
    std::cout << "🚀 Buffer Pool (LRU) + Clock Algorithm con Equivalencias 1:1" << std::endl;
    std::cout << "🎓 Educativo - Carnegie Mellon University Style" << std::endl;
    std::cout << "⚔️  Comparación Directa: LRU vs Clock Algorithm" << std::endl;
    std::cout << std::string(80, '=') << std::endl;
    ...
    while (true) {
        ...
        switch (option) {
            ...
            case 28: sistema.clockPageOperations(); break;
            case 29: sistema.createNewClockPage(); break;
            case 30: sistema.clockAdvancedPageOperations(); break;
            case 31: sistema.showClockBufferStatus(); break;
            case 32: sistema.flushAllClockPages(); break;
            case 33: sistema.compareClockVsLRUPerformance(); break;
            ...
            case 0:
                ...
                std::cout << "   • Operaciones equivalentes LRU ↔ Clock" << std::endl;
                std::cout << "   • Comparación de algoritmos" << std::endl;
                return 0;
                
            default:
                std::cout << "\n❌ Opción no válida. Selecciona 0-33." << std::endl;
                break;
        }
